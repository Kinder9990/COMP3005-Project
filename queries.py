...
def Q_1(cursor, conn, execution_time):
    ...
    query = """SELECT me.player ->> 'name' as "player",
            AVG(CAST(me.shot ->> 'statsbomb_xg' AS FLOAT)) as "avgXgScore"
            FROM match_events me
            JOIN matches m ON me.match_id = m.match_id
            JOIN competitions c ON m.competition_id = c.competition_id
            JOIN seasons s ON m.season_id = s.season_id
            WHERE me.type ->> 'name' = 'Shot' 
            AND c.competition_name = 'La Liga'
            AND s.name = '2020/2021'
            GROUP BY me.player ->> 'name'
            ORDER BY "avgXgScore" DESC;
            """
...
def Q_4(cursor, conn, execution_time):
    ...
    query = """ SELECT me.possession_details ->> 'name' as "team",
        COUNT(me.possession_details ->> 'name') as "passCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
        WHERE me.type ->> 'name' = 'Pass' 
        AND c.competition_name = 'La Liga'
        AND s.name = '2020/2021'
        GROUP BY me.possession_details ->> 'name'
...
def Q_5(cursor, conn, execution_time):
    ...
    query = """ SELECT me.pass -> 'recipient' ->> 'name' as "pass_recipient",
        COUNT(me.pass -> 'recipient' ->> 'name') as "passCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
        ...
        GROUP BY me.pass -> 'recipient' ->> 'name'
        ORDER BY "passCount" DESC; """
...
def Q_6(cursor, conn, execution_time):
    ...
    query = """SELECT  me.possession_details ->> 'name' as "team",
        COUNT(me.possession_details ->> 'name') as "shotCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
...
def Q_7(cursor, conn, execution_time):
    ...
    query = """ SELECT me.player ->> 'name' as "player",
        COUNT(me.player ->> 'name') as "throughCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
        WHERE type ->> 'name' = 'Pass' 
        and pass -> 'technique'->> 'name' = 'Through Ball' 
        AND c.competition_name = 'La Liga'
        AND s.name ='2020/2021'
        GROUP BY me.player ->> 'name'
...
def Q_8(cursor, conn, execution_time):
    ...
    query = """ SELECT me.possession_details ->> 'name' as "team",
        COUNT(me.possession_details ->> 'name') as "throughCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
...
def Q_9(cursor, conn, execution_time):
    ...
    query = """ SELECT me.player ->> 'name' as "player",
        COUNT(me.player ->> 'name') as "DribbleCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
        WHERE type ->> 'name' = 'Dribble' and dribble -> 'outcome'->> 'name' = 'Complete'
        AND c.competition_name = 'La Liga'
        AND s.name IN ('2020/2021', '2019/2020', '2018/2019')
        GROUP BY me.player ->> 'name'
        ORDER BY "DribbleCount" DESC; """
...
def Q_10(cursor, conn, execution_time):
    ...
    query = """ SELECT me.player ->> 'name' as "playerName",
       COUNT(me.player ->> 'name') as "dribblePastCount"
        FROM match_events me
        JOIN matches m ON me.match_id = m.match_id
        JOIN competitions c ON m.competition_id = c.competition_id
        JOIN seasons s ON m.season_id = s.season_id
        WHERE type ->> 'name' = 'Dribbled Past'
        AND c.competition_name = 'La Liga'
        AND s.name ='2020/2021'
        GROUP BY me.player ->> 'name'
        ORDER BY "dribblePastCount" ASC; """
